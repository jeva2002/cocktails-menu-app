...
const Cocktail: React.FunctionComponent<Props> = ({ cocktail }) => {
  ...
  return (
    <article className='cocktail border p-2 mb-2 rounded'>
      ...
      <div className='pb-2 d-flex align-items-center justify-content-evenly w-100'>
        <button className='cocktail-btn rounded-circle' disabled={amount === 0} onClick={() => setAmount(amount - 1)}>
          -
        </button>
        <b>{amount}</b>
        <button className='cocktail-btn rounded-circle' onClick={() => setAmount(amount + 1)}>+</button>
      </div>
    </article>
  );
};
